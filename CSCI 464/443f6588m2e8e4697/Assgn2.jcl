...
*       IF (DISPLACEMENT <= BESTDISP && DISPLACEMENT >0)
*           A(BRTENTRY) -> BEST@
*           DISPLACEMENT -> BESTDISP
*       ENDIF
*    ENDIF
* END FOR
*
...
**********************************************************
*GOALS: 
* CALCULATE DISPLACEMENT FOR EACH BRT ENTRY IN USE
* IN THE CASE OF THE BEST DISPLACEMENT
...
* GET VALUES FOR START, INCREMENT, TERMINATE
         LM R3,R5,=A(BRTABLE,2,BRTABLE+16*2) 
STRT01   LH R11,0(,R3)       GET THE BRT ENTRY
         CH R11,=H'0'        IF ENTRY IS EMPTY
         BE SKIPIT           SKIP THE ENTRY
* CLEAR THE SID FROM THE BRT ENTRY
         SLL R11,20          GET RID OF SID
         SRL R11,20          PUT ADDRESS BACK IN PLACE
* CALCULATE THE DISPLACEMENT
         LR R9,R7            MAKE SURE R7 STAYS CLEAN
         SR R9,R11           DISPLACEMENT -> R9
************************************************************
* IF DISPLACEMENT < 0
*     GO TO SKIPIT
* ENDIF
...
* ENDIF
************************************************************
         CR R9,R10           IF DISPLACEMENT <= BEST
         BH SKIPIT           IF DISPLACEMENT > BEST, SKIP IT
* THIS IS BEST, SAVE THE DISPLACEMENT
         XDUMP
...
*************************************************************
* AT THIS POINT BEST SHOULD BE FOUND OR ADDRESSIBILITY ERROR
* BEST IS IN R10
*************************************************************
